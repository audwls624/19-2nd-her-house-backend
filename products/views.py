    ...
    def get(self,request,product_id):
        try:
            ...
            product_info = {
                ...
                'manufacturer'      : product.manufacturer,
                'name'              : product.name,
                'star_rating'       : round(product.review_set.all().aggregate(Avg('star_rating')).get('star_rating__avg', None),1) if product.review_set.all() else None,
                'review_number'     : product.review_set.count(),
                'price'             : int(product.price),
                'discout_rate'      : product.discount_rate,
                'delivery_fee'      : int(product.delivery_fee),
